...
import {
  ConfirmationContent,
  PolicyContent,
} from "@/src/screens/PurchaseModals";
import {
  NotificationsSheet,
  ProfileMenuSheet,
} from "@/src/screens/Sheets";
...
export default function Index() {
    ...
    (async () => {
      ...
      timers.push(
        setTimeout(
          async () => {
            if (active) {
              await storage.setItem("vth-intro-seen", true);
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      const code = nextSettings.currency.supported.includes(
        savedCurrency
      )
        ? savedCurrency
        : nextSettings.currency.default;
  ...
  // ------------------------------------------------------------
  // Store configuration unavailable
  // ------------------------------------------------------------
  if (!settings) {
    return (
      <View
  ...
  // Maintenance mode
  // ------------------------------------------------------------
  if (settings.store.maintenance_mode) {
    return (
      <View
        style={[
          styles.root,
      ...
      >
        <StateBlock
          ...
          description={
            settings.store.maintenance_message
          }
  ...
  const content = selectedOrder ? (
    ...
    <HomeScreen
      ...
      onCategory={setCatalog}
      onRecharge={() => setActiveScreen("wallet")}
...
function IntroScreen({
  ...
  const copy =
    step === 0
      ...
      : step === 1
        ? {
            title: `Welcome to ${storeName.replace(
              "NETWORK",
              "Network"
            )} 👋`,
            subtitle:
              "A better way to access quality digital products.",
          }
        : {
            title: "All-in-One Account Store",
            subtitle:
              "Telegram & WhatsApp products with secure wallet payments.",
          };
    ...
    >
      ...
      <View style={styles.introFooter}>
        ...
        <Text style={styles.introMeta}>
          POWERED BY {storeName.toUpperCase()}
        </Text>
      </View>
    </LinearGradient>
  );
